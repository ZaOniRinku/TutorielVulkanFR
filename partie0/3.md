...
Il est possible de récupérer le projet de base depuis [**la branche ``debut`` du dépôt GitHub**](https://github.com/ZaOniRinku/TutorielVulkanFR/tree/debut). Celui-ci contient déjà GLFW mais vous demande tout de même d'installer **un environnement de développement C++**, **CMake** et **VulkanSDK**.
...
Récupérez la dernière version de la section *Releases* du [**dépôt GitHub de Glslang**](https://github.com/KhronosGroup/glslang/releases).
...
Récupérez la dernière version de la section *Releases* du [**dépôt GitHub de nml**](https://github.com/Team-Nutshell/nml/releases).

## stb
[**stb**](https://github.com/nothings/stb) est une bibliothèque de chargement d'images depuis des fichiers. Elle nous sera utile pour charger nos textures.

Récupérez [**stb_image.h**](https://github.com/nothings/stb/blob/master/stb_image.h) depuis le dépôt GitHub de stb.

## tinyobjloader
[**tinyobjloader**](https://github.com/tinyobjloader/tinyobjloader) est une bibliothèque de chargement de maillages depuis les fichiers OBJ.

Récupérez [**tiny_obj_loader.h**](https://github.com/tinyobjloader/tinyobjloader/blob/release/tiny_obj_loader.h) depuis le dépôt GitHub de tinyobjloader.
...
Récupérez la dernière version de la section *Releases* du [**dépôt GitHub de VulkanMemoryAllocator**](https://github.com/GPUOpen-LibrariesAndSDKs/VulkanMemoryAllocator/releases).
...
Dans celui-ci, créez un nouveau dossier ``external``, dans lequel vous allez mettre le contenu du zip téléchargé sur le site de GLFW. Celui-ci devrait avoir comme nom ``glfw-*numero.de.version*`` mais renommez-le ``glfw``.

Faites de même avec le contenu du zip téléchargé depuis le dépôt GitHub de Glslang, le dossier devrait avoir comme nom ``glslang-*numero.de.version*`` mais renommez-le ``glslang``.

Faites aussi de même avec le contenu du zip téléchargé depuis le dépôt GitHub de nml, le dossier devrait avoir comme nom ``nml-*numero.de.version*`` mais renommez-le ``nml``.

Et enfin, faites de même avec le contenu du zip téléchargé depuis le dépôt GitHub de VulkanMemoryAllocator, qui devrait avoir comme nom ``VulkanMemoryAllocator-*numero.de.version*``, renommez-le ``VulkanMemoryAllocator``.

Créez aussi un nouveau dossier ``stb`` pour y mettre ``stb_image.h`` et un dossier ``tinyobjloader`` pour y mettre ``tiny_obj_loader.h``.
...
Nous allons inclure les dossier contenant stb et tinyobjloader, ceux-ci ne contenant qu'un fichier *header* chacun.

```CMake
target_include_directories(${PROJECT_NAME} PRIVATE external/stb) # On inclut le dossier de stb
target_include_directories(${PROJECT_NAME} PRIVATE external/tinyobjloader) # On inclut le dossier de tinyobjloader
```

On va lier statiquement les libraries Vulkan, GLFW, Glslang, SPIR-V (qui est inclut dans Glslang), nml et VulkanMemoryAllocator à notre projet. Dans un projet de plus grande envergure, il est plutôt recommandé de ne pas lier Vulkan statiquement et de plutôt utiliser un meta-loader de fonctions Vulkan comme [**volk**](https://github.com/zeux/volk). Dans le cadre de ce tutoriel et pour rester au plus simple, Vulkan sera lié statiquement.
