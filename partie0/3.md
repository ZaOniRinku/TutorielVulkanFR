...
Faites aussi de même avec le contenu du zip téléchargé depuis le dépôt GitHub de VulkanMemoryAllocator, qui devrait avoir comme nom ``VulkanMemoryAllocator-*numero.de.version*``, renommez-le ``VulkanMemoryAllocator``.
...
GLFW, Glslang et VulkanMemoryAllocator possèdent eux aussi un fichier ``CMakeLists.txt``, il est donc possible d'utiliser ``add_subdirectory()`` pour les inclure à notre projet.
...
```CMake
target_link_libraries(${PROJECT_NAME} PRIVATE glfw glslang SPIRV VulkanMemoryAllocator Vulkan::Vulkan) # On lie GLFW, glslang, SPIRV (inclut dans Glslang), VulkanMemoryAllocator et Vulkan à notre projet
``` 
