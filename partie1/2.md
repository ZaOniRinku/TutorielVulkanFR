...
``messageType`` spécifie les événements qui passeront pas le messager de debug. On va l'utiliser pour les événements généraux, les messages de validation envoyés par ``VK_LAYER_KHRONOS_validation`` ainsi que les messages dits de "performance" qui nous indiquent que notre utilisation de Vulkan n'est pas optimale.
...
Il est très important de détruire le messager de debug avant de détruire l'instance puisque l'instance est utilisée dans la destruction du messager de debug. De manière générale, les objets crées à l'initialisation et détruits à la fermeture du programme doivent être détruits dans le sens inverse de leur création.
