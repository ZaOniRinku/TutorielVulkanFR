...
L'instance va notamment nous permettre d'ajouter des extensions pour les surfaces de fenêtres, et malgré que GLFW nous permette de créer automatiquement une surface Vulkan sans que l'on ait à le faire soi-même avec [**glfwCreateWindowSurface()**](https://www.glfw.org/docs/3.3/group__vulkan.html#ga1a24536bec3f80b08ead18e28e6ae965), nous n'allons pas l'utiliser ici afin d'apprendre comment créer des surfaces avec des systèmes de fenêtrages différents, voire même directement avec l'API fournie par l'OS.
...
Les fonctions de type ``vkEnumerateX`` sont appelées deux fois : une fois pour récupérer le nombre d'éléments qu'on souhaite énumérer, et une fois avec un tableau de la taille du nombre d'éléments pour les recevoir. Ici, on énumère donc tous les *layers* explicites disponibles et on vérifie si celui passé en paramètre en fait partie.
...
À chaque fois qu'un objet Vulkan est crée, celui-ci doit être détruit lorsque nous n'en avons plus besoin. Nous pouvons donc détruire l'instance Vulkan dans la méthode ``destroy()``, qui est appelée lors de la fermeture du programme :
...
On peut remarquer que l'appel à ``vkDestroyInstance`` n'est pas entouré par ``TUTORIEL_VK_CHECK()`` car les fonctions de destruction ne retournent pas de ``VkResult``.

On va maintenant créer un messager de debug avec l'extension ``VK_EXT_debug_utils`` activée lors de la création de l'instance.
...
``messageType`` spécifie les événements qui passeront par le messager de debug. On va l'utiliser pour les événements généraux, les messages de validation envoyés par ``VK_LAYER_KHRONOS_validation`` ainsi que les messages dits de "performance" qui nous indiquent que notre utilisation de Vulkan n'est pas optimale.
...
Il est très important de détruire le messager de debug avant de détruire l'instance puisque l'instance est utilisée dans la destruction du messager de debug. De manière générale, les objets créés à l'initialisation et détruits à la fermeture du programme doivent être détruits dans le sens inverse de leur création.
