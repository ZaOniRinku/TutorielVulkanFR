...
[**``VkPhysicalDeviceProperties2``**](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkPhysicalDeviceProperties2.html) est l'exemple d'une fonctionnalité qui était disponible via une extension ([**``VK_KHR_get_physical_device_properties2``**](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VK_KHR_get_physical_device_properties2.html)) mais qui fait maintenant partie du cœur de Vulkan 1.1. Elle a le chiffre ``2`` à la fin de son nom car il existe [**``VkPhysicalDeviceProperties``**](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkPhysicalDeviceProperties.html). La deuxième version ajoute un ``pNext`` permettant de l'étendre afin de récupérer des informations additionnelles.
...
On peut donc ajouter un entier 32 bits non signé à la classe ``HelloTriangle`` pour y mettre le numéro de la famille de queues supportant les opérations graphiques et le rendu à l'écran :
...
On a aussi la mention d'extensions de *device*, avec ``enabledExtensionCount`` et ``ppEnabledExtensionNames``.
...
On peut ensuite récupérer la queue avec un nouvel attribut de la classe ``HelloTriangle`` :
