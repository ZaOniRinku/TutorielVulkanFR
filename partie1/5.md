...
Ainsi, si une image est de taille 100x100, son niveau de mipmap 0 est de taille 100x100, le niveau 1 est de taille 50x50 et le niveau 2 est de taille 25x25. Nous ne pouvons plus diviser la taille par 2 et tomber sur une valeur entière, l'image aurait donc 3 niveaux de mipmap. C'est pourquoi les largeurs et hauteurs des textures sont généralement des puissances de 2, leur permettant d'avoir un niveau de mipmap final ayant la taille 1x1, donc un seul pixel.
...
On peut maintenant passer à la création de la *swapchain*, dans une fonction ``createSwapchain`` dans la classe ``RenderingEngine`` :
