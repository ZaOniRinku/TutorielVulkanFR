...
Le layout de pipeline crée, nous ne devons pas oublier de le supprimer à la fermeture du programme dans la fonction ``destroy()`` :
...
Directement après avoir crée notre pipeline graphique, nous pouvons détruire nos modules de *shaders*, puisqu'ils ne nous serviront plus maintenant que la pipeline graphique a été créée :
