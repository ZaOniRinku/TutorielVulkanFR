...
``vkAcquireNextImageKHR`` est aussi une fonction bloquante, et le CPU sortira soit après avoir reçu un indice d'une image de la *swapchain*, soit après 6 siècles, nous considérons donc, ici, que si le CPU bloque sur cette fonction, notre programme a probablement un bug.
...
L'exécution des commandes débute dans l'ordre (donc ici, ``Commande 0`` va commencer, puis ``Commande 1`` va commencer, puis ``Commande 2`` va commencer, etc.) mais s'exécutent en parallèle ensuite, ce qui nous donne aucune garantit sur l'ordre de fin des commandes. Dans cet exemple, ``Commande 5`` peut se finir en premier, puis ``Commande 3`` peut se terminer, puis ``Commande 2``, etc.
...
``dstStageMask`` est un [**``VkPipelineStageFlags2``**](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/VkPipelineStageFlagBits2.html) qui donne l'étape de pipeline où l'image va être utilisé ensuite. Nous allons écrire dessus en tant qu'image de couleur attachée à une passe de rendu, nous mettons donc ``VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT``.
...
Nous devons spécifier un *scissor*, qui est celui que nous avons défini lors de la création de la pipeline graphique et qui couvre l'entièreté de l'image, puis nous devons donner les images attachées à la passe de rendu.
...
``dstStageMask`` donne l'étape de pipeline où l'image va être utilisé ensuite. Nous allons la présenter à l'écran, nous mettons donc ``VK_PIPELINE_STAGE_2_NONE``.
...
Nous allons ensuite dé-signaler notre *Fence* pour que le CPU bloque sur l'appel de ``vkWaitForFences`` au début de la fonction ``update()``, tant que celle-ci n'a pas été signalée par le GPU :
...
``waitSemaphoreCount`` et ``pWaitSemaphores`` spécifient les sémaphores sur lesquelles le GPU va attendre avant d'exécuter les commandes. ``m_acquireCompletedSemaphores[m_currentFrameInFlight]`` est la sémaphore que nous voulons signaler avec ``vkAcquireNextImageKHR``, et qui sera signalée lorsque le GPU pourra de nouveau utiliser cette image.
...
``signalSemaphoreCount`` et ``pSignalSemaphores`` sont les sémaphores qui seront signalées lorsque toutes les commandes auront complété leur exécution.
...
``waitSemaphoreCount`` et ``pWaitSemaphores`` sont les sémaphores sur lesquelles le GPU va attendre. Ici, le GPU va attendre que la sémaphore ``m_renderCompletedSemaphores[imageIndex]`` soit signalée, ce qui arrivera quand toutes les commandes auront fini leur exécution, tel que décrit dans notre appel à ``vkQueueSubmit``. Nous ne voulons pas présenter une image qui n'a pas terminé son rendu.
...
À noter que [**``vkQueueWaitIdle``**](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/vkQueueWaitIdle.html) et [**``vkDeviceWaitIdle``**](https://registry.khronos.org/vulkan/specs/1.3-extensions/man/html/vkDeviceWaitIdle.html) (qui effectue un ``vkQueueWaitIdle`` sur toutes les queues d'un *device* logique) ne doivent pas remplacer les *Fences* et les sémaphores car leur utilisation non-réfléchie ralentiront le programme.

[**Chapitre précédent**](8.md) - [**Index**](../index.md) - [**Chapitre suivant**](10.md)

[**Code de la partie**](https://github.com/ZaOniRinku/TutorielVulkanFR/tree/partie1/9)