...
Puisque nous avons tous nos objets dans le même *buffer*, nous devons nous déplacer dans la mémoire de la taille d'une matrice modèle pour passer à l'objet suivant, c'est ce que fait la variable ``offset``, qui sera égal à 0 pour le premier objet, 64 pour le deuxième, 128 pour le troisième, etc.
...
Nous pouvons maintenant remplacer la commande de dessin que nous utilisions pour le triange pour dessiner nos objets :
...
- l'addresse vers les données à passer, qui ici est l'indice de l'objet en train d'être dessiné.
