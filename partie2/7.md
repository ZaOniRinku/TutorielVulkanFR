...
``anisotropyEnable`` permet d'activer le filtrage anisotrope. Le filtrage anistrope permet de modifier la texture selon l'angle avec laquelle elle est regardée, en échantillonnant plusieurs valeurs, ce nombre étant spécifié par ``maxAnisotropy`` (qui ici signifie donc d'échantillonner au maximum 16 valeurs par pixel).
...
[**Chapitre précédent**](6.md) - [**Index**](../index.md)
