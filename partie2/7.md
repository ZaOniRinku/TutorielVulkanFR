...
Nous modifions aussi la ``range`` du *Storage Buffer* des informations sur les objets car nous allons ajouter l'indice de texture. Bien que celui-ci sera un ``uint32_t``, nous ajoutons la taille d'un ``nml::vec4`` pour aligner correctement les données en mémoire.
