...
Le filtre ``VK_FILTER_LINEAR`` est généralement utilisé, mais il existe aussi ``VK_FILTER_NEAREST`` qui ne fait pas d'interpolation mais qui va récupérer la valeur du pixel le plus proche. C'est le filtre à utiliser sur les textures en pixel-art, par exemple, car nous ne voulons pas interpoler les pixels entre eux.
