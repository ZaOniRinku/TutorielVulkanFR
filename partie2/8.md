...
Pour cela, nous allons utiliser la bibliothèque **stb** qui prend en charge de nombreux formats de fichiers.
...
Nous allons supprimer le code qui créait une texture avec une boucle ``for`` ainsi que l'appel de la fonction ``createTexture`` dans la fonction ``init``.
...
``stbi_load`` prend en paramètre l'adresse vers l'image à charger, un pointeur vers la largeur, un pointeur vers la hauteur, un pointeur vers le nombre de canaux (4 pour RBGA) et les canaux que nous souhaitons charger. Elle retourne l'image sous forme d'un tableau de caractères non signés.
...
Pour pallier ce problème, nous allons créer une texture de 1 pixel blanc. Comme exemple de texture présente pour signifier que le modèle n'a pas de texture, le moteur de jeu [**Source**](https://developer.valvesoftware.com/wiki/Source) de Valve et Minecraft utilisent une texture en damier noir et rose.
...
Cette image sera forcément la première de la liste des textures, et sera donc l'indice 0, nous pouvons donc l'assigner par défaut en tant qu'indice de texture à la structure ``Object``, dans ``renderingengine.h`` :
